...
export class Client {
    ...
    #quic: WebTransport | undefined;
    #statInterval: number | undefined;
    stats: { time: number; payload: any }[] = [];
    ...
    async connect(): Promise<Connection> {
        ...
        this.#quic = new WebTransport(this.config.url, options);
        await this.#quic.ready;

        const stream = await this.#quic.createBidirectionalStream();
        ...
        const objects = new Objects(this.#quic);

        // @ts-expect-error: getStats is an experimental method
        if (this.#quic && this.#quic.getStats) {
