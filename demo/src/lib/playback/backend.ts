...
export default class Backend {
    ...
    constructor(config: PlayerConfig) {
        ...
        for (const track of config.catalog.tracks) {
            if (isAudioTrack(track)) {
                if (
                    sampleRate &&
                    track.selectionParams.samplerate !== sampleRate
                ) {
                    throw new Error(
                        `TODO multiple audio tracks with different sample rates`
                    );
                }
                ...
                channels = Math.max(
                    +track.selectionParams.channelConfig,
                    channels ?? 0
                );
            }
        }
    ...
    public async communicate(message: Message.ToWorker) {
        return new Promise((resolve) => {
            ...
            const handler = (e: MessageEvent) => {
                ...
                // Get only the data from the message
                delete e.data.id;
